    ...
    IOHIPointing__PASecondaryParameters* secondaryParams)
{
    ...
    success = ((primaryParams->gain[0].value != 0LL) || (primaryParams->gain[1].value != 0LL) || (primaryParams->gain[2].value != 0LL) || (primaryParams->gain[3].value != 0LL));
        ...
        for (type = kAccelTypeY; type <= kAccelTypeZ; type++) {
            ...
            if (res) {
                ...
                if (parametricAccelerationCurves && CFGetTypeID(parametricAccelerationCurves) == CFArrayGetTypeID() && reportRate) {
                    ...
                    _scrollWheelInfo.axis[type].isParametric = PACurvesSetupAccelParams(parametricAccelerationCurves,
                        desired64,
                        devScale64,
                        scrScale64,
...
static void scaleWheel(int* deltaAxis1, SInt32* fixedDeltaAxis1, SInt32* pointDeltaAxis1, struct timespec ts)
{
    ...
    for (UInt32 type = kAccelTypeY; type <= kAccelTypeZ; type++) {
        directionChange[type] = ((_scrollWheelInfo.axis[type].lastValue == 0) || ((_scrollWheelInfo.axis[type].lastValue < 0) && (*(pDeltaAxis[type]) > 0)) || ((_scrollWheelInfo.axis[type].lastValue > 0) && (*(pDeltaAxis[type]) < 0)));
        ...
        if (_scrollWheelInfo.axis[type].scaleSegments || _scrollWheelInfo.axis[type].isParametric) {
            ...
            *(pScrollFixedDeltaAxis[type]) = *(pScrollPointDeltaAxis[type]) << 16;
            ...
            if (_scrollWheelInfo.axis[type].consumeCountThreshold) {
                ...
                if (*(pScrollFixedDeltaAxis[type]) && ((abs(_scrollWheelInfo.axis[type].lastValue) >= (SInt32)_scrollWheelInfo.axis[type].consumeClearThreshold) || (_scrollWheelInfo.axis[type].consumeState.consumeCount >= _scrollWheelInfo.axis[type].consumeCountThreshold))) {
